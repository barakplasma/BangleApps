...
//
// The approved set below lists every global Espruino 2.x provides.
// Add an entry only when espruino.com/Features documents the global, with
// the version it appeared in. Do NOT add globals that Espruino lacks just
// to silence the test — fix the bundle instead.
//
// EXCLUDED (must NOT appear in the bundle):
//   Reflect         — not in Espruino
...
const APPROVED_GLOBALS = new Set([
  ...
  // ── Typed arrays (supported since Espruino 1v80) ──────────────────────────
  ...
  'Set',     // 2v00
  'Promise', // 2v00
  'Symbol',  // 1v96 (with '@@iterator' string fallback in Babel output)
  'Intl',    // 2v22 — used for DateTimeFormat timezone formatting

  // ── Polyfilled by the app before the bundle runs ──────────────────────────
  'Temporal', // @hebcal/core uses Temporal.ZonedDateTime for molad times
